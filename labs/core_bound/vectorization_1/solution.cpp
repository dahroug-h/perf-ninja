...
// Transpose the DNA seq matrix
simd_sequence_t transpose(std::vector<sequence_t> const &sequence) {
  assert(sequence.size() == sequence_count_v);

  simd_sequence_t simd_seq;
  for (size_t i = 0; i < simd_seq.size(); i++) {
    for (size_t j = 0; j < sequence.size(); j++) {
      simd_seq[i][j] = sequence[j][i];
    }
  }
...
result_t compute_alignment(std::vector<sequence_t> const &sequences1,
  ...
  auto transeq1 = transpose(sequences1);
  auto transeq2 = transpose(sequences2);
  ...
  /*
    ...
    */
  for (unsigned col = 1; col <= transeq2.size(); ++col) {
    ...
    for (unsigned row = 1; row <= transeq1.size(); ++row) {
      // Compute next score from diagonal direction with match/mismatch.
      score_t best_cell_score = last_diagonal_score;
      for (size_t k = 0; k < sequence_count_v; ++k) {
        best_cell_score[k] += (transeq1[row - 1][k] == transeq2[col - 1][k]) ? match[k] : mismatch[k];
      }
      for (size_t k = 0; k < sequence_count_v; ++k) {
