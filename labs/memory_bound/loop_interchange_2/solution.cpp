...
static void filterVertically(uint8_t *output, const uint8_t *input,
  ...
  for (int r = 0; r < std::min(radius, height); r++) {
    ...
    for(int c = 0; c < width; c++){
      value[c] = static_cast<int>(dot[c] / static_cast<float>(sum[c]) + 0.5f);
      output[r * width + c] = static_cast<uint8_t>(value[c]);
    }
  ...
  //   for (int c = 0; c < width; c++) {
  //   // Fast shift instead of division
  //     output[r * width + c] = static_cast<uint8_t>( (dot[c] + rounding) >> shift );
  //   }
  // }
  ...
  for (int r = std::max(radius, height - radius); r < height; r++) {
    ...
    // Normalization
    for(int c = 0; c < width; ++c){
      output[r * width + c] = static_cast<uint8_t>(static_cast<int>(dot[c] / static_cast<float>(sum[c]) + 0.5f));
    }
  }
